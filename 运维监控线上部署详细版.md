...
###一、python3.6.7的离线安装
	...
	首先将：
		...
		放到/opt/django下，这两个是安装python3.6.7所需要的依赖
...
###二、安装项目的python依赖包
	...
	依赖包线上安装：
		...
		2、`cd /opt/django/site-packages`
		
		3、 通过pip3可执行命令执行一下命令来安装python3的依赖包：
...
###三、Django项目源代码部署
	...
	1、cd /opt/django 文件夹下，找到项目源代码service_manager.tar.gz包
